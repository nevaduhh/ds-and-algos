...
# First approach
def count_overlapping(intervals, num):
  overlapping_sum = 0
...
# Cleaner approach
def count_overlapping_2(intervals, num):
  return len([interval for interval in intervals if num >= interval[0] and num <= interval[1]])

# Destructured sum approach
def count_overlapping_3(intervals, num):
  return sum([begin <= num <= end for begin, end in intervals])
